...
`data-a11y-label` 只填写区域短名称，例如 `要闻`，无需写成 `要闻视窗区` 或完整提示句；旧页面兼容属性 `aria-readlabel` 遵循同一规则。用户通过快捷键或工具栏进行分类导航时，工具会按 `区域名称 + 分类名称` 直接拼接，并补充 Tab 操作提示，例如播报“提示：您已进入要闻视窗区，按下 Tab 键浏览信息；第 1 个，共 2 个”。工具不裁剪名称空白，也不判断或去除名称中已有的分类词，因此 `主导航` 与 `导航区` 会生成 `主导航导航区`；接入方应按期望提供短名称。普通 Tab 聚焦区域只更新焦点与活动区域状态，不额外播报这条进入提示。
