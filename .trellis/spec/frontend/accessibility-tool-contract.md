...
- Both crosshair axes use the themeable `--a11y-danger` color (default red) with a light/dark contrast outline. They remain fixed, `aria-hidden`, and `pointer-events: none`, so they never intercept host-page pointing, scrolling, or focus.
