...
- Runtime scope: browser-native TypeScript. The main runtime excludes `opencc-js` and `pinyin-pro` implementation code/data; those dependencies are built into two fixed-name, same-directory ESM language assets and loaded only when large-caption presentation needs them. Desktop layouts start at 1024px.
...
dist/accessibility-tool-opencc.js -> ESM, self-contained OpenCC caption asset
dist/accessibility-tool-pinyin.js -> ESM, self-contained pinyin caption asset
dist/accessibility-tool.css     -> external CSP stylesheet
dist/index.d.ts                  -> public TypeScript entry declaration
```

Host HTML references exactly one chosen main entry, normally `dist/accessibility-tool.min.js`; it never adds script tags for the language assets. Both the IIFE and ESM main entries dynamically import `./accessibility-tool-opencc.js?v=<package-version>` and `./accessibility-tool-pinyin.js?v=<package-version>` as fixed, same-directory ESM assets. An integration's runtime is an atomic three-file set: its chosen main entry plus both language assets. Deploying a new main file without its matching two assets, deleting either asset, or mixing versions is invalid. When both main formats are published, all four JavaScript artifacts are released together. `dist/accessibility-tool.css` remains an optional strict-CSP integration artifact rather than a dependency of the default IIFE.

For the default embedded-style integration, the two language imports are the only automatic tool-originated runtime resource requests allowed by v0.2. They resolve against the selected main entry's deployment source and may not fall back to another origin, third-party CDN, API, dictionary endpoint, or cloud service. An explicitly configured strict-CSP `styleUrl` remains governed by the styling boundary above and is never a language-asset fallback. Initial production asset URLs carry the package version. A failed import is not cached as success: the next demand retries the same fixed asset and version with an appended monotonic `retry=<attempt>` query, while concurrent demand for one attempt shares a single promise and a successful module remains cached. Caption/page text and derived conversion or pinyin data must never enter an asset URL, query, request body/header, log, telemetry event, or persistent storage.
...
- Only in that development mode, `/accessibility-tool.min.js` resolves to `src/index.ts`, response-time HTML transformation promotes existing local defer scripts to modules, and caption language imports resolve through the source modules under `src/language/`. Demo JavaScript and tool source therefore participate in Vite's module graph and update handling. Source HTML files are never rewritten.
- Vite build keeps the project root, library formats, fixed filenames, declaration generation, external stylesheet emission, and `public` copying unchanged. Vite preview remains a `dist` production preview: it loads the two versioned production language assets beside the selected main entry and must never receive the development root, aliases, source-language-module URLs, or HTML transformation.
...
- Large-caption controls expose mutually exclusive `简体` / `繁体`, independent `拼音`, and mutually exclusive `28` / `36` / `48` pixel choices. The immutable original text is rendered synchronously before asynchronous presentation work. The first effective caption presentation loads the OpenCC asset on demand; the pinyin asset is requested only while pinyin is enabled. `opencc-js` standard `cn2t` / `t2cn` conversion and tone-marked `pinyin-pro` output are then derived from that immutable snapshot on every setting change, so switching modes never converts an already converted result. Mixed Chinese/non-Chinese content preserves its source order, and rendering uses DOM text nodes rather than library HTML or `innerHTML`. An asset-load or converter failure preserves the original written text; a pinyin load/generation failure or character-alignment mismatch drops only the pinyin presentation. Neither failure emits a public error, alters the speech input, or interrupts output, and a later demand may retry a failed asset.
...
| OpenCC asset load/conversion fails, or the pinyin asset load/generation fails/cannot align with display characters | Preserve the original written text; omit only the invalid pinyin presentation; do not emit a public error, alter speech input, or stop the session. Clear only the failed import attempt so later demand retries its same asset/version with the next `retry` query. |
...
- Good (caption): the host HTML loads one main entry, deploys its two matching fixed-name language assets beside it, starts a caption-only continuous session when speech is unavailable, and exposes the current text through one named, focusable, non-live region. The original appears immediately; simplified, traditional, and tone-marked pinyin presentations arrive from independently loaded modules and are regenerated from one immutable source snapshot, while an older output token cannot hide or overwrite a newer segment.
...
- Bad (caption): announcing the full caption through `aria-live` / `role="status"`, rendering library HTML with `innerHTML`, persisting page text, chaining conversions from already converted text, coupling the caption toggle to audio, putting text in an import URL/request/log, directly referencing language assets from host HTML, using unfixed/generated chunk names, or falling back to a deployment source different from the main entry is forbidden.
...
- Unit: caption presentation covers standard `cn2t` / `t2cn`, mixed Chinese/Latin/numeric/punctuation order, tone-marked pinyin, immutable-source round trips, immediate original-text rendering, safe text-node rendering, hidden phonetic annotations, and load/converter/pinyin/alignment failure fallbacks. OpenCC and pinyin loaders independently cover on-demand timing, concurrent single-flight behavior, successful caching, failed-attempt eviction, stable version queries, monotonic retry queries, and stale async-result suppression after text, preference, visibility, or lifecycle changes.
...
- E2E: Chrome and Edge cover both toolbar entries, audio-independent single-text output, hidden-until-effective behavior, viewport-bottom non-live presentation, simplified/traditional conversion, tone-marked pinyin, 48px sizing, synchronized state and persistence, explicit-close focus return, reload/navigation restoration without stored body text, and long-text reflow at a 640px narrow fixture with forced-colors emulation. Production-bundle request capture must prove phased OpenCC then pinyin loading, no duplicate request after success, original-text fallback plus independently retryable 5xx failures, stable version plus incremented retry queries, same-deployment-source URLs with no caption text, and strict-CSP loading of both production assets. The suite does not treat that narrow fixture as a supported mobile/tablet production layout.
...
- Build plus release-artifact inspection: both main entries reference `./accessibility-tool-opencc.js?v=<package-version>` and `./accessibility-tool-pinyin.js?v=<package-version>` but contain no OpenCC/pinyin vendor payload. The release contains only the two expected main JavaScript entries and the two fixed-name, self-contained ESM language assets, with no third language/data chunk. Each selectable main-entry trio is atomically deployable, the default IIFE works without the optional CSS file, and the main IIFE remains within the recorded raw/gzip budget.
- Serving inspection: a true dev server dynamically resolves OpenCC and pinyin through their `src/language/` modules, while Vite preview resolves only the versioned, fixed-name production assets next to the production main entry; neither mode may cross into the other's URL contract.
- E2E: Chrome and Edge run one continuous session across main/read-screen modes, verify focus preservation, pause replay, editable/reference privacy, generic and dynamic content, route/modal/disabled/lifecycle stops, stale-callback suppression, and shared metadata. Language-network inspection permits only the two same-deployment-source assets when demanded and rejects remote speech/vendor calls, alternate language sources, or any request containing page/caption text.
...
// Duplicates spoken content for screen readers, trusts generated markup, leaks
// caption text into a request, and bypasses the fixed same-source asset names.
caption.setAttribute("aria-live", "polite");
caption.innerHTML = renderPinyinHtml(convertCaption(caption.innerHTML));
const converter = await import(
  `https://other.example/opencc.js?text=${encodeURIComponent(caption.textContent ?? "")}`,
);
...
// The main runtime first renders the immutable original, then its internal
// loader independently imports the fixed, versioned same-directory OpenCC and
// pinyin assets. Load/conversion failure keeps this text and never affects audio.
captionText.replaceChildren(document.createTextNode(originalText));
const openCCRequest = captionLanguages.loadOpenCC().catch(() => null);
const pinyinRequest = pinyinEnabled
  ? captionLanguages.loadPinyin().catch(() => null)
  : null;
